...
export default function App(){
  ...
  useEffect(()=>{ 
    if (jobs) ensureOrders(jobs.map(j=>j.id), trackedIds) 
  },[jobs, trackedIds])

  const rowsForExport = useMemo(()=>{ 
    const arr: string[][] = [['id','title','company','location','status','appliedAt','url']]; 
    (jobs||[]).forEach(j=>{ 
      const p = state.items[j.id]; 
      if (!p?.status || p.status==='saved') return; 
      arr.push([j.id, text(j.title), text(j.company), text(j.location), String(p.status), String(p.appliedAt||''), text(j.url)]) 
    }); 
    return arr 
  },[jobs,state])

  if(error){ 
    return <div className='container mx-auto max-w-3xl p-6'>
      <p className='mb-2 text-red-600'>Error: {error}</p>
      {rawSample && <pre className='mt-3 rounded bg-slate-100 p-2 text-xs text-slate-700 dark:bg-slate-900 dark:text-slate-300'>{rawSample}</pre>}
    </div> 
  }
  ...
  return (
    <div className='min-h-screen'>
      ...
      {view==='search' && (<SearchPage jobs={jobs} state={state} setState={updateItem} 
        filters={{ q,setQ, region,setRegion, role,setRole, onlyNew,setOnlyNew, hideApplied,setHideApplied, sort,setSort }} />)}
