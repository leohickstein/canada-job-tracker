...
type Props = { 
  jobs: Job[]; 
  state: PersistState; 
  setState: (id:string, patch:any)=>void; 
  filters: { 
    ...
    hideApplied: boolean; setHideApplied:(b:boolean)=>void; 
...
export function SearchPage({ jobs, state, setState, filters }: Props){
  ...
  }, [filters.q, filters.region, filters.role, filters.onlyNew, filters.hideApplied, filters.sort])

  // Wrapper functions that trigger loading state
  const handleFilterChange = (filterName: string, value: any) => {
    startTransition(() => {
      switch(filterName) {
        case 'q': filters.setQ(value); break;
        case 'region': filters.setRegion(value); break;
        case 'role': filters.setRole(value); break;
        case 'onlyNew': filters.setOnlyNew(value); break;
        case 'hideApplied': filters.setHideApplied(value); break;
        case 'sort': filters.setSort(value); break;
      }
    })
  }
        ...
        <div className="container mx-auto max-w-7xl px-4 py-4">
          <div className="text-center space-y-2">
            <div className="flex items-center justify-center gap-3">
              <div className="p-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 shadow-lg">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
              Discover your next opportunity from thousands of remote and local positions.
            </p>
            ...
            <div className="flex items-center justify-center gap-4 text-xs pt-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-600 dark:text-slate-400">
                  <span className="font-semibold text-emerald-600 dark:text-emerald-400">{jobs.length.toLocaleString()}</span> total jobs
                </span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-blue-400 to-indigo-500"></div>
                <span className="text-slate-600 dark:text-slate-400">
                  <span className="font-semibold text-blue-600 dark:text-blue-400">{filtered.length.toLocaleString()}</span> matching
                  {showLoading && <Loader2 className="inline ml-1 h-3 w-3 animate-spin text-blue-500" />}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <aside className="col-span-12 lg:col-span-3">
            <div className="sticky top-6">
              <FiltersPanel 
                q={filters.q} setQ={(v) => handleFilterChange('q', v)}
                region={filters.region} setRegion={(v) => handleFilterChange('region', v)}
                role={filters.role} setRole={(v) => handleFilterChange('role', v)}
                onlyNew={filters.onlyNew} setOnlyNew={(v) => handleFilterChange('onlyNew', v)}
                hideApplied={filters.hideApplied} setHideApplied={(v) => handleFilterChange('hideApplied', v)}
                sort={filters.sort} setSort={(v) => handleFilterChange('sort', v)}
              ...
              {showLoading && (
                <div className="absolute -top-4 right-0 z-10">
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-blue-500/20 to-indigo-500/20 dark:from-blue-400/20 dark:to-indigo-400/20 backdrop-blur-sm border border-blue-300/30 dark:border-blue-500/30 shadow-lg">
                    ...
                    <span className="text-xs font-medium text-blue-700 dark:text-blue-300">Filtering...</span>
                  </div>
                </div>
              )}
              {/* Results Grid */}
              <div className={`transition-all duration-300 ${showLoading ? 'opacity-60' : 'opacity-100'}`}>
                {showLoading ? (
                  /* Skeleton Cards */
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {Array.from({ length: 6 }).map((_, i) => (
                      <div key={i} className="group relative overflow-hidden rounded-2xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] min-h-[320px] flex flex-col">
                        {/* Gradient overlay */}
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {filtered.map(j=> (
                      <JobCard key={j.id} job={j as any} persist={state.items[j.id]}
                        onOpen={()=>setState(j.id, { clickedAt: new Date().toISOString(), tracked: true })}
                        onApply={()=>setState(j.id, { status: 'applied', appliedAt: new Date().toISOString(), tracked: true })}
                        onSave={()=>setState(j.id, { status: state.items[j.id]?.status === 'saved' ? undefined : 'saved' })}
                        onTrack={(v)=>setState(j.id, { tracked: v })}
                        onNotes={(t)=>setState(j.id, { notes: t, tracked: true })}
                      />
                    ))}
                  </div>
                ) : (
                  ...
                  <div className='relative overflow-hidden rounded-3xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 dark:border-slate-700/30 shadow-lg p-12'>
                    ...
                    <div className="relative text-center space-y-4">
                      ...
                      <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300">No jobs match your criteria</h3>
                      <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                        Try adjusting your filters or search terms to see more results. There are {jobs.length.toLocaleString()} total jobs available.
                      </p>
                    </div>
                  </div>
                )}
