...
import type { Job, Persist, JobWithAnalysis } from "@/types/job";
...
export function JobCard({
  ...
  const chips = [regionOf(job), roleOf(job)]
    .filter(Boolean)
    .slice(0, 2) as string[];
  const extraChipCount =
    [regionOf(job), roleOf(job)].filter(Boolean).length - chips.length;
  ...
  const getCardStyling = () => {
    if (hasStatus) {
      switch (persist?.status) {
        case "applied":
          return "ring-2 ring-blue-500/30 bg-blue-50/50 dark:bg-blue-950/20";
        case "interview":
          return "ring-2 ring-purple-500/30 bg-purple-50/50 dark:bg-purple-950/20";
        case "offer":
          return "ring-2 ring-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-950/20";
        case "rejected":
          return "ring-2 ring-red-500/30 bg-red-50/50 dark:bg-red-950/20";
      }
    }
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-2xl backdrop-blur-xl border border-white/20 dark:border-slate-700/30 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02] hover:bg-white/90 dark:hover:bg-slate-900/90 ${getCardStyling()}`}
    >
      ...
      <div className="relative p-5 flex flex-col h-full min-h-[320px]">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2 mb-3 min-h-[24px]">
            <div className="flex items-center gap-2">
              ...
              {hasStatus && persist?.status && (
                <StatusBadge status={persist.status} />
              )}
            </div>
        ...
        {/* Salary Analysis - Phase 1 Feature */}
        {job.salaryAnalysis && (
          <div className="mb-4">
            <SalaryBadge analysis={job.salaryAnalysis} compact={true} />
          </div>
        )}
        ...
        {onNotes && showNotes && (
          <div className="mb-4">
            <textarea
              ...
              onFocus={() => {
                if (onTrack && !isTracked) onTrack(true);
              }}
            />
          </div>
        )}
